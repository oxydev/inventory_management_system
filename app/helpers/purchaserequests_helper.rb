module PurchaserequestsHelper
  ...
  def admin_user
    ...
    if current_user.admin
      links += link_to("Approve", user_purchaserequests_path(current_user), :class => "btn btn-success")
      links += " "
      links += link_to("Reject", user_purchaserequests_path(current_user), :class => "btn btn-danger")
    else
